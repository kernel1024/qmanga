...
{
    QMenu cm(this);
    QAction* nt = new QAction(QIcon(":/img/16x16/zoom-fit-best"),tr("Zoom fit"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(setZoomFit()));
    cm.addAction(nt);
    nt = new QAction(QIcon(":/img/16x16/zoom-fit-width"),tr("Zoom width"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(setZoomWidth()));
    cm.addAction(nt);
    nt = new QAction(QIcon(":/img/16x16/zoom-fit-height"),tr("Zoom height"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(setZoomHeight()));
    cm.addAction(nt);
    nt = new QAction(QIcon(":/img/16x16/zoom-original"),tr("Zoom original"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(setZoomOriginal()));
    cm.addAction(nt);
    cm.addSeparator();
    nt = new QAction(QIcon(":/img/16x16/zoom-draw.png"),tr("Zoom dynamic"),NULL);
    nt->setCheckable(true);
    nt->setChecked(zoomDynamic);
    connect(nt,SIGNAL(triggered(bool)),this,SLOT(setZoomDynamic(bool)));
    cm.addAction(nt);
    cm.addSeparator();
    nt = new QAction(QIcon(":/img/16x16/view-refresh"),tr("Redraw page"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(redrawPage()));
    cm.addAction(nt);
    cm.addSeparator();
    nt = new QAction(QIcon(":/img/16x16/document-close"),tr("Close manga"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(closeFileCtx()));
    cm.addAction(nt);
    nt = new QAction(QIcon(":/img/16x16/view-preview"),tr("Set page as cover"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(changeMangaCoverCtx()));
    cm.addAction(nt);
    nt = new QAction(QIcon(":/img/16x16/folder-tar"),tr("Export pages to directory"),NULL);
    connect(nt,SIGNAL(triggered()),this,SLOT(exportPagesCtx()));
    cm.addAction(nt);
    cm.addSeparator();
    nt = new QAction(QIcon(":/img/16x16/go-down"),tr("Minimize window"),NULL);
